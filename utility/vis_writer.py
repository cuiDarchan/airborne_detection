...
class VisWriter:
    ...
    def _init_video_writer(self, frame_width=2448, frame_height=2048, mode = 'GRAY'):
        self._video_path = os.path.join(self._output_path, self._file_name)
        if mode == 'GRAY':
            self._video_writer = cv2.VideoWriter(str(self._video_path), cv2.VideoWriter_fourcc(*'mp4v'), self._fps,
                                            (int(frame_width), int(frame_height)), 0) # 最后一个参数是灰度
        elif mode == 'RGB':
            self._video_writer = cv2.VideoWriter(str(self._video_path), cv2.VideoWriter_fourcc(*'mp4v'), self._fps,
